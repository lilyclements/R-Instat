...
DataSheet$set("public","split_date", function(col_name = "", year_val = FALSE, year_name = FALSE, leap_year = FALSE,  month_val = FALSE, month_abbr = FALSE, month_name = FALSE, week_val = FALSE, week_abbr = FALSE, week_name = FALSE,  weekday_val = FALSE, weekday_abbr = FALSE, weekday_name = FALSE,  day = FALSE, day_in_month = FALSE, day_in_year = FALSE, day_in_year_366 = FALSE, pentad_val = FALSE, pentad_abbr = FALSE,  dekad_val = FALSE, dekad_abbr = FALSE, quarter_val = FALSE, quarter_abbr = FALSE, with_year = FALSE, s_start_month = 1, s_start_day_in_month = 1, days_in_month = FALSE) {
  ...
  if(weekday_name) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = weekday_name_vector, adjacent_column = org_name)
  }
  if(weekday_abbr) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = weekday_abbr_vector, adjacent_column = org_name)
  }
  if(weekday_val) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = weekday_val_vector, adjacent_column = org_name)
  }
  if(week_val) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = week_Val_vector, adjacent_column = org_name)
  }
  if(pentad_abbr) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = pentad_abbr_vector, adjacent_column = org_name)
  }
  if(pentad_val) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = pentad_val_vector, adjacent_column = org_name)
  }
  if(dekad_abbr) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = dekad_abbr_vector, adjacent_column = org_name)
  }
  if(dekad_val) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = dekad_val_vector, adjacent_column = org_name)
  }
  if(quarter_val) {
    if(s_shift) {
      ...
      self$add_columns_to_data(col_name = col_name, col_data = s_quarter_val_vector, adjacent_column = org_name)
      ...
      self$add_columns_to_data(col_name = col_name, col_data = quarter_val_vector, adjacent_column = org_name)
    }
  ...
  if(day_in_year) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = day_in_year_vector, adjacent_column = org_name)
    self$append_to_variables_metadata(col_names = col_name, property = doy_start_label, new_val = s_start_day)
  ...
  if(day_in_year_366) {
    if(s_shift) {
      ...
      self$add_columns_to_data(col_name = col_name, col_data = temp_s_doy, adjacent_column = org_name)
      ...
      self$add_columns_to_data(col_name = col_name, col_data = day_in_year_366_vector, adjacent_column = org_name)
    }
  ...
  if(days_in_month) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = days_in_month_vector, adjacent_column = org_name)
  }
  if(day_in_month) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = day_in_month_vector, adjacent_column = org_name)
  ...
  if(month_val) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = month_val_vector, adjacent_column = org_name)
  ...
  if(month_abbr) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = month_abbr_vector, adjacent_column = org_name)
    if(s_shift) self$append_to_variables_metadata(col_names = col_name, property = label_label, new_val = paste("Shifted month starting on day", s_start_day))
    if(is_climatic && is.null(self$get_climatic_column_name(month_label))) {
      self$append_climatic_types(types = c(month = col_name))
    }
  ...
  if(month_name) { 
    ...
    self$add_columns_to_data(col_name = col_name, col_data = month_name_vector, adjacent_column = org_name)
    if(s_shift) self$append_to_variables_metadata(col_names = col_name, property = label_label, new_val = paste("Shifted month starting on day", s_start_day))
    if(is_climatic && is.null(self$get_climatic_column_name(month_label))) {
      self$append_climatic_types(types = c(month = col_name))
    }
  ...
  if(year_name) {
    if(s_shift) {
      ...
      self$add_columns_to_data(col_name = col_name, col_data = temp_s_year, adjacent_column = org_name)
      ...
      self$add_columns_to_data(col_name = col_name, col_data = factor(year_vector), adjacent_column = org_name)
    }
  ...
  if(year_val) {
    if(s_shift) {
      col_name <- next_default_item(prefix = "s_year", existing_names = self$get_column_names(), include_index = FALSE)
      self$add_columns_to_data(col_name = col_name, col_data = temp_s_year_num, adjacent_column = org_name)
      self$append_to_variables_metadata(col_names = col_name, property = label_label, new_val = paste("Shifted year starting on day", s_start_day))
    }
    else {
      year_vector <- lubridate::year(col_data)
      col_name <- next_default_item(prefix = "year", existing_names = self$get_column_names(), include_index = FALSE)
      self$add_columns_to_data(col_name = col_name, col_data = year_vector, adjacent_column = org_name)
    }
    if(is_climatic && is.null(self$get_climatic_column_name(year_label))) {
      self$append_climatic_types(types = c(year = col_name))
    }
  ...
  if(leap_year) {
    ...
    self$add_columns_to_data(col_name = col_name, col_data = leap_year_vector, adjacent_column = org_name)
  }
 }
)
